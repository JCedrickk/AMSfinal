            ...
            <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <p class="text-sm text-[#4a5568] mt-0.5">View all users with administrative privileges</p>
                    </div>
                </div>
            ...
            <table class="w-full">
                ...
                <tbody class="divide-y divide-gray-200">
                    ...
                    <tr class="hover:bg-gray-50 transition">
                        <td class="px-6 py-4 text-sm text-[#1a2a4a] font-medium">{{ $admin->id }}</td>
                        <td class="px-6 py-4">
                            <div class="flex items-center gap-2">
                                @if($admin->profile && $admin->profile->profile_picture)
                                    <img src="{{ asset('storage/' . $admin->profile->profile_picture) }}" 
                                         alt="Profile" 
                                ...
                                @endif
                                <span class="text-sm font-medium text-[#1a2a4a]">{{ $admin->first_name }} {{ $admin->last_name }}</span>
                                @if($admin->id == auth()->id())
                                    <span class="text-xs bg-blue-100 text-blue-700 px-1.5 py-0.5 rounded">You</span>
                                @endif
                            </div>
                        </td>
                        <td class="px-6 py-4 text-sm text-[#4a5568]">{{ $admin->email }}</td>
                        <td class="px-6 py-4">
                        ...
                        <td class="px-6 py-4 text-sm text-[#4a5568]">{{ $admin->created_at->format('M d, Y') }}</td>
                        <td class="px-6 py-4">
                            @if($admin->id != auth()->id())
                                <button type="button" class="px-3 py-1.5 bg-orange-600 text-white rounded-lg text-xs font-semibold hover:bg-orange-700 transition-all flex items-center gap-1" 
                                        onclick="openRemoveAdminModal({{ $admin->id }})">
                                    <i class="fas fa-user-minus"></i> Remove Admin
                                </button>
                            @else
                                <span class="text-xs text-gray-400">Cannot remove self</span>
                            @endif
                            
                            <!-- Remove Admin Modal -->
                            <div id="removeAdminModal{{ $admin->id }}" class="fixed inset-0 z-50 hidden overflow-y-auto" aria-labelledby="modal-title" role="dialog" aria-modal="true">
                                <div class="flex items-center justify-center min-h-screen px-4">
                                    <div class="fixed inset-0 bg-gray-900/60 backdrop-blur-sm transition-opacity" onclick="closeRemoveAdminModal({{ $admin->id }})"></div>
                                    ...
                                    <div class="relative bg-white rounded-2xl max-w-md w-full mx-auto shadow-xl transform transition-all">
                                        ...
                                        <form action="{{ route('admin.users.remove-admin', $admin) }}" method="POST" onsubmit="return validateRemoveAdminForm({{ $admin->id }})">
                                            @csrf
                                            <div class="p-6">
                                                ...
                                                <label class="flex items-center gap-2 cursor-pointer">
                                                    <input type="checkbox" id="confirmRemove{{ $admin->id }}" class="w-4 h-4 text-red-600 rounded border-gray-300 focus:ring-red-500" required>
                                            ...
                                            <div class="px-6 py-4 border-t border-gray-200 flex justify-end gap-3">
                                                <button type="button" class="px-4 py-2 bg-gray-100 text-[#4a5568] rounded-lg font-semibold text-sm hover:bg-gray-200 transition" 
                                                        onclick="closeRemoveAdminModal({{ $admin->id }})">Cancel</button>
                                                <button type="submit" class="px-4 py-2 bg-red-600 text-white rounded-lg font-semibold text-sm hover:bg-red-700 transition opacity-50 cursor-not-allowed" 
                                                        id="removeAdminBtn{{ $admin->id }}" disabled>Remove Admin Privileges</button>
                                            </div>
                                        </form>
                                    </div>
                                </div>
                        ...
                        <td colspan="7" class="px-6 py-12 text-center">
                            <div class="flex flex-col items-center gap-2">
                                <div class="w-16 h-16 bg-gray-100 rounded-full flex items-center justify-center">
                                    <i class="fas fa-user-shield text-2xl text-[#2c3e66]"></i>
                                </div>
                                <p class="text-[#1a2a4a] font-medium">No administrators found</p>
                                <p class="text-sm text-[#4a5568]">There are no admin users in the system.</p>
                            </div>
                        </td>
                    </tr>
                    @endforelse
